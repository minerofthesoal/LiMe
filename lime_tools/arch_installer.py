...
class ArchAutoInstaller:
    ...
    def detect_hardware(self) -> Dict:
        ...
        print(f"GPU Vendor: {hardware['gpu_vendor']}")
    ...
    def _driver_packages_for_hardware(self, hardware: Dict) -> List[str]:
        """Select additional driver packages based on detected hardware."""
        gpu = hardware.get("gpu_vendor", "unknown")
        packages: List[str] = []
        if gpu == "nvidia":
            packages.extend(["nvidia", "nvidia-utils"])
        elif gpu == "amd":
            packages.extend(["mesa", "xf86-video-amdgpu", "vulkan-radeon"])
        elif gpu == "intel":
            packages.extend(["mesa", "xf86-video-intel", "vulkan-intel"])
        else:
            packages.append("mesa")

        if hardware.get("has_wifi"):
            packages.append("iwd")
    ...
    def install_lime_os_complete(self) -> bool:
        ...
        # Hardware-aware package tuning
        auto_driver_packages = self._driver_packages_for_hardware(hardware)
        for package in auto_driver_packages:
            if package not in self.pacstrap_packages:
                self.pacstrap_packages.append(package)
        print(f"Auto-selected driver packages: {', '.join(auto_driver_packages)}")
